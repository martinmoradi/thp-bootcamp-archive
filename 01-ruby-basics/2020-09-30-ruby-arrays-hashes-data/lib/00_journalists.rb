...
puts
puts "Il y a #{five_chars(handles_twitter)} handles contenant 5 caractères !"
puts
puts "#############################################################"
puts
puts "Combien commencent par une majuscule (première lettre juste après le @) ?"
puts
def first_char_uppercase?(handles_twitter)
    i = 0
    alpha = "azertyuiopmlkjhgfdsqwxcvbn"
        handles_twitter.each do |handle|
            if alpha.upcase.include?(handle[1]) #handle[1] #== handle[1].upcase && 
                i += 1
            end
        end
    return i
...
puts "Il y a #{first_char_uppercase?(handles_twitter)} handles qui commencent par une majuscule !"
puts
puts "#############################################################"
puts
puts "Trie la liste de handle par ordre alphabétique."
puts
def sort_alphabetically(handles_twitter)
    return handles_twitter.sort
end

print "Voici l'array : #{sort_alphabetically(handles_twitter)}"
puts
puts
puts "#############################################################"
puts
...
puts
def sort_small_first(handles_twitter)
    return handles_twitter.sort_by(&:length)
end
print "Voici l'array : #{sort_small_first(handles_twitter)}"
puts 
puts
puts "#############################################################"
puts
...
def find_index(handles_twitter, element)
    return handles_twitter.find_index(element) + 1 
end

puts "L'élément @epenser est en position n°#{find_index(handles_twitter, "@epenser")} ! "
puts
puts "#############################################################"
puts
...
puts
puts "Voici les handles réparties : #{sort_handles(handles_twitter).sort}"
