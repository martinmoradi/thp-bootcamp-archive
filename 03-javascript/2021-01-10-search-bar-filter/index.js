...
const allKeywords = keywordsCategories.reduce(
  (prevKeywords, category) => [...prevKeywords, ...category.keywords],
  []
);
...
const addNewKeyword = (label, keyword) => {
  ...
  if (!label || !keyword) {
    console.error(
      "It seems you forgot to write the label or keyword in the addNewKeyword function"
    );
    return;
  }
...
const cleanedKeyword = (keyword) => {
  const cleanedKeyword = keyword.toLowerCase().replace(/[^\w\s]/gi, "");
